# PSBT Howto for Fortress Core

Since Fortress Core 0.17, an RPC interface exists for Partially Signed Fortress
...
PSBT is an interchange format for Fortress transactions that are not fully signed
...
Overall, the construction of a fully signed Fortress transaction goes through the
following steps:
...
- An **Extractor** produces a valid Fortress transaction (in network format)
...
#### Multisig with multiple Fortress Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Fortress Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `fortress-cli` is used.
